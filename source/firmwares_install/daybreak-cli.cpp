...
bool DaybreakInitializeMenu() {
	...
	if (R_FAILED(rc = splGetConfig(static_cast<SplConfigItem>(ExosphereApiVersionConfigItem), &version))) {
		printf(language_vars["lng_db_atmosphere_not_found_error"].c_str(), rc);
		printf("\n");
		custom_pause();
		return false;
	}
	...
	if (!ams_supports_sysupdate_api) {
		printf(language_vars["lng_db_atmosphere_outdated_error"].c_str(), rc);
		printf("\n");
		custom_pause();
		return false;
	}
	...
	if (envIsNso()) {
		printf(language_vars["lng_db_nso_launch_error"].c_str(), rc);
		printf("\n");
		custom_pause();
		return false;
	}
...
Result GetUpdateInformation() {
	...
	if (R_FAILED(rc = amssuGetUpdateInformation(&m_update_info, g_update_path))) {
		if (rc == 0x1a405) {
			printf(language_vars["lng_db_update_not_found_error"].c_str(), rc);
			printf("\n");
		} else {
			printf(language_vars["lng_db_get_update_infos_error"].c_str(), rc);
			printf("\n");
		}
		custom_pause();
		return rc;
	...
	if (m_update_info.exfat_supported) {
		printf(language_vars["lng_db_update_infos_exfat_support"].c_str());
		printf("\n");
	} else {
		printf(language_vars["lng_db_update_infos_exfat_no_support"].c_str());
		printf("\n");
	}
	printf(language_vars["lng_db_update_infos_fw_version_variations"].c_str(), m_update_info.num_firmware_variations);
	printf("\n");
	consoleUpdate(&logs_console);
...
void ValidateUpdate() {
	...
	if (R_FAILED(rc = amssuValidateUpdate(&m_validation_info, g_update_path))) {
		printf(language_vars["lng_db_update_validation_error"].c_str(), rc);
		printf("\n");
		custom_pause();
		return;
	}
	...
	if (R_SUCCEEDED(m_validation_info.result)) {
		printf(language_vars["lng_db_update_validation_success"].c_str());
		printf("\n");

		if (R_FAILED(m_validation_info.exfat_result)) {
		   const u32 version = m_validation_info.invalid_key.version;
			printf(language_vars["lng_db_update_validation_exfat_error"].c_str(), m_validation_info.exfat_result);
			printf("\n");
			printf(language_vars["lng_db_update_validation_missing_content"].c_str(), m_validation_info.invalid_key.id, (version >> 26) & 0x1f, (version >> 20) & 0x1f, (version >> 16) & 0xf);
			printf("\n");
		...
		const u32 version = m_validation_info.invalid_key.version;
		printf(language_vars["lng_db_update_validation_content_error"].c_str(), m_validation_info.result);
		printf("\n");
		printf(language_vars["lng_db_update_validation_missing_content"].c_str(), m_validation_info.invalid_key.id, (version >> 26) & 0x1f, (version >> 20) & 0x1f, (version >> 16) & 0xf);
		printf("\n");
...
Result TransitionUpdateState() {
	...
	if (m_install_state == InstallState::NeedsSetup) {
		...
		if (R_FAILED(rc = amssuSetupUpdate(nullptr, UpdateTaskBufferSize, g_update_path, g_use_exfat))) {
			printf(language_vars["lng_db_install_process_setup_error"].c_str(), rc);
			printf("\n");
			MarkForReboot();
			return rc;
		}
		...
		printf(language_vars["lng_db_install_process_setup_success"].c_str());
		printf("\n");
		...
		if (R_FAILED(rc = amssuRequestPrepareUpdate(&m_prepare_result))) {
			printf(language_vars["lng_db_install_process_request_preparation_error"].c_str(), rc);
			printf("\n");
			MarkForReboot();
			return rc;
		}
		...
		printf(language_vars["lng_db_install_process_preparing"].c_str());
		printf("\n");
		...
		if (R_FAILED(rc = asyncResultWait(&m_prepare_result, 0)) && rc != 0xea01) {
			printf(language_vars["lng_db_install_process_get_preparation_result_error"].c_str(), rc);
			printf("\n");
			MarkForReboot();
			return rc;
		} else if (R_SUCCEEDED(rc)) {
			if (R_FAILED(rc = asyncResultGet(&m_prepare_result))) {
				printf(language_vars["lng_db_install_process_preparation_error"].c_str(), rc);
				printf("\n");
				MarkForReboot();
				return rc;
			}
		...
		if (R_FAILED(rc = amssuHasPreparedUpdate(&prepared))) {
			printf(language_vars["lng_db_install_process_preparation_result_error"].c_str(), rc);
			printf("\n");
			MarkForReboot();
			return rc;
		}
		...
		if (prepared) {
			printf("\n");
			printf(language_vars["lng_db_install_process_preparation_success"].c_str());
			printf("\n");
			printf(language_vars["lng_db_install_process_applying_update"].c_str());
			printf("\n");
		...
		if (R_FAILED(rc = amssuGetPrepareUpdateProgress(&update_progress))) {
			printf(language_vars["lng_db_install_process_get_progress_error"].c_str(), rc);
			printf("\n");
			MarkForReboot();
			return rc;
		}
		...
		if (R_FAILED(rc = amssuApplyPreparedUpdate())) {
			printf(language_vars["lng_db_install_process_applying_update_error"].c_str(), rc);
			printf("\n");
		} else {
			/* Log success. */
			printf(language_vars["lng_db_install_process_success"].c_str());
			printf("\n");
			...
			if (g_reset_to_factory) {
				...
				printf(language_vars["lng_db_install_process_reset_to_factory_success"].c_str(), rc);
				printf("\n");
			}
		}
...
bool daybreak_main(char *current_path, int force_reset_to_factory, int force_exfat, int force_unsuported_firmware) {
	...
	if (is_erista && has_rcm_bug_patch && !is_emummc) {
		printf(language_vars["lng_db_patched_model_warning"].c_str());
		printf("\n");
		consoleUpdate(&logs_console);
	}
	...
	if (!R_FAILED(GetUpdateInformation())) {
		...
		printf(language_vars["lng_db_update_validation"].c_str());
		printf("\n");
		consoleUpdate(&logs_console);
	}
	...
	if (g_exfat_supported && force_exfat == 1) {
		...
		g_use_exfat = daybreak_ask_question((char *) language_vars["lng_db_ask_exfat_driver_install"].c_str());
	} else {
		g_use_exfat = false;
	}
	bool choose_fat32_forced = false;
	if (!g_exfat_supported && g_use_exfat) {
		choose_fat32_forced = daybreak_ask_question((char *) language_vars["lng_db_ask_fat32_if_exfat_not_supported"].c_str());
	...
	if (EncodeVersion((version >> 26) & 0x1f, (version >> 20) & 0x1f, (version >> 16) & 0xf) > g_supported_version) {
		if (force_unsuported_firmware == 1) {
			printf(language_vars["lng_db_force_unsupported_firmware_enabled"].c_str());
			printf("\n");
			consoleUpdate(&logs_console);
			force_update_not_supported = true;
		} else if (force_unsuported_firmware == 0) {
			force_update_not_supported = daybreak_ask_question((char *) language_vars["lng_db_ask_force_unsupported_firmware_install"].c_str());
		} else {
	...
	*/
	if (force_reset_to_factory == 1) {
		printf(language_vars["lng_db_reset_to_factory_forced"].c_str());
		printf("\n");
		g_reset_to_factory = true;
	} else if (force_reset_to_factory == 0) {
		g_reset_to_factory = daybreak_ask_question((char *) language_vars["lng_db_ask_reset_to_factory"].c_str());
	} else {
	...
	if (m_install_state == InstallState::NeedsDraw) {
		printf(language_vars["lng_db_install_update_begin"].c_str());
		printf("\n");
		consoleUpdate(&logs_console);
